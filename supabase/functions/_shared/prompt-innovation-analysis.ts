...
The user message may include a \`## The Beacon Ecosystem — Real Context For This Prospect\` section with real use case themes, events, service descriptions, and reference analyses pulled from The Beacon's database. When that section is present you MUST use the specific named companies, use cases, and events it contains — never default to generic phrases like "The Beacon's AI/ML specialists" or "our ecosystem can help":
...
- **\`innovation_opportunities[i].beacon_connection\`** MUST name at least one specific Beacon member company from the Use Case Themes (e.g. "Timefold's scheduling optimization", "ML6's forecasting models", "Dockflow's real-time container tracking") and describe what that member has already built that directly addresses this opportunity. Do NOT write generic sentences like "The Beacon's AI/ML specialists can provide expertise" — that's a failure. If no named member in the provided context fits the opportunity, write a shorter \`beacon_connection\` that references a specific event or use case theme by title instead.
- **\`recommended_offerings[i].offering\`** must match a title from the Service Offerings list verbatim; \`match_reason\` must explain (a) the specific value of this tier for this prospect, referencing concrete benefits from the service description, and (b) a named member or use case theme the prospect would meet through it. Not "helps you explore partnerships" but "Explore Partnership (€5,000) gives you structured access to the Smart Ports use case themes — Timefold, ML6, NxtPort — through 3 Innovation Briefings per year, which is exactly the intake format that matched Borealis to their chemical-logistics partners".
